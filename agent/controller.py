...
class CodingAgent:
    ...
    def run(self, task: str) -> AgentState:
        ...
        self._initialize_issue_context(state)
    ...
    def _preferred_source_path(state: AgentState) -> str | None:
        ...
        for path in CodingAgent._issue_candidate_paths(state.task):
            if not _looks_like_test_file(path):
                return path
    ...
    def _fallback_arguments(step: dict[str, Any], state: AgentState) -> dict[str, Any]:
        ...
        if tool_name == "search_files":
            for candidate in CodingAgent._issue_candidate_paths(state.task):
                if candidate.endswith(".py") and not _looks_like_test_file(candidate):
    ...
    def _initialize_issue_context(self, state: AgentState) -> None:
        """Preload source files explicitly named in the task."""
        if self.tool_registry.get("read_file") is None:
            return

        candidate_paths = self._issue_candidate_paths(state.task)
        ...
        for path in candidate_paths:
            if path in seen or _looks_like_test_file(path):
                continue
            seen.add(path)
            try:
            ...
            for imported_path in imported_paths:
                if imported_path not in seen and not _looks_like_test_file(imported_path):
                    seen.add(imported_path)
                    candidate_paths.append(imported_path)

    @staticmethod
    def _add_retrieval_context(task: str, state: AgentState) -> str:
        sections = [task]
        retrieval_context = CodingAgent._retrieval_text(state, max_length=1200)
        if retrieval_context and retrieval_context != "暂无":
            sections.append(f"相关代码检索上下文:\n{retrieval_context}")
        issue_context = CodingAgent._build_debug_context(state, max_length=1200)
        if issue_context:
            sections.append(f"已定位源码上下文:\n{issue_context}")
        return "\n\n".join(sections)

    @staticmethod
    def _issue_candidate_paths(task: str) -> list[str]:
        candidates = extract_python_paths(task)
        seen: set[str] = set()
        paths: list[str] = []
        ...
        fallback = re.findall(r"[A-Za-z_][A-Za-z0-9_./-]*\.py", task)
